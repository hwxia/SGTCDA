...
def dense2sparse(matrix: np.ndarray):
    mat_coo = coo_matrix(matrix)  # matrix的维度为(218, 271) 该方法用于创建一个稀疏矩阵
    edge_idx = np.vstack((mat_coo.row, mat_coo.col))  # mat_coo.row包含了稀疏矩阵中非零元素的行坐标; mat_coo.col包含了稀疏矩阵中非零元素的列坐标。
    return edge_idx, mat_coo.data  # edge_idx中第一行为非零元素的药物行坐标，第二行为非零元素的circRNA列坐标

def construct_het_mat(rna_dis_mat, dis_mat, rna_mat):  # rna_dis_mat表示Drug-circRNA的关联矩阵 dis_mat表示整合后的circRNA的相似度矩阵 rna_mat表示整合后的药物相似度矩阵
    mat1 = np.hstack((rna_mat, rna_dis_mat))
    mat2 = np.hstack((rna_dis_mat.T, dis_mat))
    ret = np.vstack((mat1, mat2))
    return ret

def construct_adj_mat(training_mask):  # training_mask为 Drug-circRNA的关联矩阵
    adj_tmp = training_mask.copy()
    rna_mat = np.zeros((training_mask.shape[0], training_mask.shape[0]))  # rna_mat大小为(218,218) 元素全为0
    dis_mat = np.zeros((training_mask.shape[1], training_mask.shape[1]))  # dis_mat大小为(271,271) 元素全为0
...
def calculate_loss(pred, pos_edge_idx, neg_edge_idx):  # pred表示经过特征矩阵相乘且经过sigmoid后的结果, pos_edge_idx表示正样本的索引, neg_edge_idx表示负样本的索引
    pos_pred_socres = pred[pos_edge_idx[0], pos_edge_idx[1]]  # 正样本的预测分数
    neg_pred_socres = pred[neg_edge_idx[0], neg_edge_idx[1]]  # 负样本的预测分数
    pred_scores = torch.hstack((pos_pred_socres, neg_pred_socres))  # 表示总共的预测分数
    true_labels = torch.hstack((torch.ones(pos_pred_socres.shape[0]), torch.zeros(neg_pred_socres.shape[0])))  # 表示真实值 大小为8268
...
def calculate_evaluation_metrics(pred_mat, pos_edges, neg_edges):
    ...
    # np.save('SGT_pred(test).npy', pred_labels)
    # np.save('SGT_true(test).npy', true_labels)
...
def load_data():
    ...
    drug_sim=pd.read_csv("../data/drug_str_sim.csv", index_col=0, dtype=np.float32).to_numpy()  # index_col=0表示第一列作为索引
...
def construct_train_dataset(train_pos_edges, train_neg_edges):
    num_columns = train_pos_edges.shape[1]  # num_columns=3307
    tmp_train_pos_edges = []
    tmp_train_neg_edges = []
    for col_index in range(num_columns):
        current_column = np.append(train_pos_edges[:, col_index], 1)
        tmp_train_pos_edges.append(current_column)

    for col_index in range(num_columns):
        current_column = np.append(train_neg_edges[:, col_index], 0)
        tmp_train_neg_edges.append(current_column)

    train_dataset = np.concatenate((tmp_train_pos_edges, tmp_train_neg_edges), axis=1)

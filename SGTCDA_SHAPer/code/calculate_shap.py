...
from drug_edgeshaper import drug_edgeshaper
from circ_edgeshaper import circ_edgeshaper
from asso_edgeshaper import asso_edgeshaper

def calculate_shap(edge_idx_dict, drug_dis_matrix, n_drug, n_cir, drug_sim, cir_sim, args_config, device):
    lr = args_config['lr']
    ...
    # # Calculate the out-degree of drug and circRNA nodes
    # drug_adj_new = drug_adj
    # cir_adj_new = cir_adj
    # drug_adj_new[drug_adj_new != 0] = 1
    # cir_adj_new[cir_adj_new != 0] = 1
    # drug_out_degree = np.sum(drug_adj_new, axis=0)
    # cir_out_degree = np.sum(cir_adj_new, axis=0)
    #
    # df1 = pd.DataFrame(drug_out_degree, columns=["drug_degree"])
    # df2 = pd.DataFrame(cir_out_degree, columns=["cir_degree"])
    #
    # result_df = pd.concat([df1, df2], axis=1)
    # result_df.to_excel("./drugANDcir_degree.xlsx", index=False)
    ...
    # Calculate the shap value of the Drug edge
    model = torch.load('../model/model_best.pth')
    model.to(device)
    drug_edge_shap, drug_i, drug_j = drug_edgeshaper(model, het_mat_device, edge_idx_device, embeddings_drug,
                                     edge_idx_drug, embeddings_cir, edge_idx_cir, drug_dis_matrix, M=100, P=None,
                                     seed=42)
    df1 = pd.DataFrame(drug_edge_shap, columns=["Drug-Drug-shap"])
    df2 = pd.DataFrame(drug_i, columns=["drug_i"])
    df3 = pd.DataFrame(drug_j, columns=["drug_j"])
    result_df = pd.concat([df1, df2, df3], axis=1)
    result_df.to_excel("../shap_result/drug_shap_value.xlsx", index=False)


    # Calculate the shap value of the circRNA edge
    model = torch.load('../model/model_best.pth')
    model.to(device)
    circ_edge_shap, cir_i, cir_j = circ_edgeshaper(model, het_mat_device, edge_idx_device, embeddings_drug,
                                     edge_idx_drug, embeddings_cir, edge_idx_cir, drug_dis_matrix, M=100, P=None,
                                     seed=42)
    df1 = pd.DataFrame(circ_edge_shap, columns=["circ-circ-shap"])
    df2 = pd.DataFrame(cir_i, columns=["cir_i"])
    df3 = pd.DataFrame(cir_j, columns=["cir_j"])
    result_df = pd.concat([df1, df2, df3], axis=1)
    result_df.to_excel("../shap_result/circ_shap_value.xlsx", index=False)


    # Calculate the shap values of the edges in the association matrix
    model = torch.load('../model/model_best.pth')
    model.to(device)
    asso_edge_shap, asso_i, asso_j = asso_edgeshaper(model, het_mat_device, edge_idx_device, embeddings_drug,
                                                   edge_idx_drug, embeddings_cir, edge_idx_cir, drug_dis_matrix, M=1,
                                                   P=None,
                                                   seed=42)
    df1 = pd.DataFrame(asso_edge_shap, columns=["drug-circ-shap"])
    df2 = pd.DataFrame(asso_i, columns=["asso_i"])
    df3 = pd.DataFrame(asso_j, columns=["asso_j"])
    result_df = pd.concat([df1, df2, df3], axis=1)
    result_df.to_excel("../shap_result/asso_shap_value.xlsx", index=False)
